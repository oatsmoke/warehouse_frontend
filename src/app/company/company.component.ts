...
export class CompanyComponent implements OnInit {
  ...
  getCompanies() {
    this.companyService.getAll(true).pipe(first()).subscribe((value: any) => {
  ...
  dialogDeleteCompany(id: number) {
    this.dialog.open(DialogCompanyDeleteRestore, {data: this.globalService.delete}).afterClosed().pipe(first()).subscribe(value => {
      if (value) {
  ...
  dialogRestoreCompany(id: number) {
    this.dialog.open(DialogCompanyDeleteRestore, {data: this.globalService.restore}).afterClosed().pipe(first()).subscribe(value => {
      if (value) {
        this.companyService.restore(id).pipe(first()).subscribe({
          next: _ => {
...
@Component({
  selector: 'dialog-company-delete-restore', templateUrl: './dialog-company-delete-restore.html'
})
export class DialogCompanyDeleteRestore {
}
